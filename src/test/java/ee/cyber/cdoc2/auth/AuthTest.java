...
import static ee.cyber.cdoc2.auth.TestData.TEST_MLDSA_CERT_ISSUER_PEM;
import static ee.cyber.cdoc2.auth.TestData.TEST_MLDSA_ALGORITHM;

class AuthTest {

    static Logger log = LoggerFactory.getLogger(AuthTest.class);

    private static final Date TEST_VALIDATION_DATE = createTestValidationDate();

    private static Date createTestValidationDate() {
        try {
            return new SimpleDateFormat("yyyy-MM-dd").parse("2025-06-01");
        } catch (Exception e) {
            throw new IllegalStateException("Failed to create test validation date", e);
        }
    }

    private static final Date TEST_VALIDATION_DATE_MLDSA = createTestValidationDateMldsa();

    private static Date createTestValidationDateMldsa() {
        try {
            return new SimpleDateFormat("yyyy-MM-dd").parse("2026-04-21");
        } catch (Exception e) {
            throw new IllegalStateException("Failed to create ML-DSA test validation date", e);
        }
    }
    ...
    void testGenerateVerifyTicketWithRsaKey() throws Exception {
        ...
        AuthTokenVerifier tokenVerifier = new AuthTokenVerifier(
                TestData.createTestIssuerTrustStore(List.of(TEST_RSA_CERT_ISSUER_PEM)),
                false,
    ...
    void testGenerateVerifyTicketWithEcdsaKey() throws Exception {
        ...
        AuthTokenVerifier tokenVerifier = new AuthTokenVerifier(
                TestData.createTestIssuerTrustStore(List.of(TEST_ECDSA_CERT_ISSUER_PEM)),
                false,
                TEST_VALIDATION_DATE
        );
        Map<String, Object> verifiedClaims = tokenVerifier.getVerifiedClaims(
    ...
    @Test
    void testGenerateVerifyTicketWithMldsaKey() throws Exception {
        EtsiIdentifier recipient = new EtsiIdentifier("etsi/PNOEE-" + TestData.TEST_IDENTIFIER);

        String sdjwt0 = TestData.generateTestAuthTicketWithMldsaKey(
                recipient.getSemanticsIdentifier(),
                "https://css.ria.ee:443",
                "9EE90F2D-D946-4D54-9C3D-F4C68F7FFAE3",
                "59b314d4815f21f73a0b9168cecbd5773cc694b6"
        ...
        log.debug("ML-DSA SDJWT 0: {}", sdjwt0);

        AuthTokenVerifier tokenVerifier = new AuthTokenVerifier(
                TestData.createTestIssuerTrustStore(List.of(TEST_MLDSA_CERT_ISSUER_PEM)),
                false,
                TEST_VALIDATION_DATE_MLDSA
        );

        Map<String, Object> verifiedClaims = tokenVerifier.getVerifiedClaims(
                sdjwt0,
                TestData.loadDefaultTestCert(TEST_MLDSA_ALGORITHM)
        );

        log.debug("verified ML-DSA claims from ticket: {}", verifiedClaims);
    ...
    @Test
    void shouldRejectTamperedMldsaAuthTicket() throws Exception {
        EtsiIdentifier recipient = new EtsiIdentifier("etsi/PNOEE-" + TestData.TEST_IDENTIFIER);

        String ticket = TestData.generateTestAuthTicketWithMldsaKey(
                recipient.getSemanticsIdentifier(),
                "https://css.ria.ee:443",
                "9EE90F2D-D946-4D54-9C3D-F4C68F7FFAE3",
                "59b314d4815f21f73a0b9168cecbd5773cc694b6"
        ...
        String[] sdJwtParts = ticket.split("~", -1);
        String jws = sdJwtParts[0];
        ...
        String tamperedJws = jwsParts[0] + "." + jwsParts[1] + "." + tamperedSignature;

        StringBuilder tamperedTicket = new StringBuilder(tamperedJws);
        for (int i = 1; i < sdJwtParts.length; i++) {
            tamperedTicket.append("~").append(sdJwtParts[i]);
        }
        ...
        assertThrows(
                ...
                () -> tokenVerifier.getVerifiedClaims(
                        tamperedTicket.toString(),
                        TestData.loadDefaultTestCert(TEST_MLDSA_ALGORITHM)
                )
        );
    }
    ...
    @Test
    void shouldRejectMldsaTicketWhenIssuerIsNotTrusted() throws Exception {
        EtsiIdentifier recipient = new EtsiIdentifier("etsi/PNOEE-" + TestData.TEST_IDENTIFIER);
        ...
        KeyStore emptyTrustStore = TestData.createEmptyTrustStore();
    ...
    @Test
    void shouldRejectMldsaTicketWithWrongCertificate() throws Exception {
        EtsiIdentifier recipient = new EtsiIdentifier("etsi/PNOEE-" + TestData.TEST_IDENTIFIER);
        ...
        assertThrows(
                ...
                () -> tokenVerifier.getVerifiedClaims(
                        ticket,
                        TestData.loadDefaultTestCert(SID_PUBLIC_KEY_ALGORITHM)
                )
        );
    }
