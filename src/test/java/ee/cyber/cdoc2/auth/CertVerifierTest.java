...
class CertVerifierTest {
    ...
    private static final Date TEST_VALIDATION_DATE =
            Date.from(Instant.parse("2025-06-01T00:00:00Z"));

    @Test
    void shouldVerify() throws Exception {
        CertVerifier verifier = new CertVerifier(
                TestData.createTestIssuerTrustStore(
                        List.of(TEST_RSA_CERT_ISSUER_PEM)),
                false,
    ...
    void shouldNotVerifyWhenTrustStoreDoesNotContainIssuer() throws Exception {
        ...
        CertVerifier verifier = new CertVerifier(selfTrustStore, true, TEST_VALIDATION_DATE);
