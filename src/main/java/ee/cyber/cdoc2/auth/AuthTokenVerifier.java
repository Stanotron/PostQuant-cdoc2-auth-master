...
public class AuthTokenVerifier {
    ...
    private static final JWSAlgorithm MLDSA_JWS_ALGORITHM = new JWSAlgorithm("MLDSA");
    ...
    public AuthTokenVerifier(KeyStore issuersTrustStore, boolean enableRevocationChecks) {
        this(issuersTrustStore, enableRevocationChecks, null);
    }
    ...
    public AuthTokenVerifier(
        ...
        this.certVerifier = new CertVerifier(issuersTrustStore, enableRevocationChecks, validationDate);
    ...
    private Map<String, Object> getVerifiedClaimsByKeyAlgorithm(
        ...
        String normalizedAlgorithm = normalizeKeyAlgorithm(cert.getPublicKey());

        log.debug("Certificate public key algorithm: {}", publicKeyAlgorithm);
        log.debug("Normalized certificate public key algorithm: {}", normalizedAlgorithm);

        if ("RSA".equals(normalizedAlgorithm)) {
            return getVerifiedClaimsUsingRSACert(cert, token);
        } else if ("EC".equals(normalizedAlgorithm)) {
            return getVerifiedClaimsUsingECDSACert(cert, token);
        } else if ("MLDSA".equals(normalizedAlgorithm) || isMlDsaAlgorithm(publicKeyAlgorithm)) {
            return getVerifiedClaimsUsingMLDSACert(cert, token);
        } else {
            throw new VerificationException(
                    "Expected certificate public key to be RSA, EC, or MLDSA algorithm, but was "
                            + publicKeyAlgorithm
            );
        }
    }
    ...
    private static boolean isMlDsaAlgorithm(String algorithm) {
        ...
        return normalized.equals("MLDSA")
                || normalized.equals("ML-DSA")
                || normalized.startsWith("ML-DSA-")
                || normalized.startsWith("MLDSA");
    }

    private static String normalizeKeyAlgorithm(PublicKey publicKey) {
        String alg = publicKey.getAlgorithm();
        if (alg == null) {
            return "";
        }

        String normalized = alg.replace("-", "").replace("_", "").toUpperCase(Locale.ROOT);

        if ("RSA".equals(normalized)) {
            return "RSA";
        }
        if ("EC".equals(normalized) || "ECDSA".equals(normalized)) {
            return "EC";
        }
    ...
    private Map<String, Object> getVerifiedClaimsUsingMLDSACert(X509Certificate cert, String token)
            throws VerificationException, ParseException, JOSEException {

        // For prototype PQ certs, we keep using the same semantics-identifier extraction logic
        String subjectSerial = extractKIDFunc.apply(cert);
        PublicKey publicKey = cert.getPublicKey();

        return getVerifiedClaimsForMLDSA(token, subjectSerial, publicKey);
    }
    ...
    private static Map<String, Object> getVerifiedClaimsForMLDSA(
            String token,
            String pubKeyId,
            PublicKey publicKey
    ) throws VerificationException, JOSEException, ParseException {

        JWSVerifier jwsVerifier = createMLDSAVerifier(publicKey, pubKeyId);
        return getVerifiedClaims(
                token,
                pubKeyId,
                jwsVerifier,
                Set.of(MLDSA_JWS_ALGORITHM)
        );
    }
    ...
    private static JWSVerifier createMLDSAVerifier(
            PublicKey publicKey,
            String pubKeyId
    ) throws VerificationException {
        if (pubKeyId == null) {
            throw new VerificationException("Expected kid for MLDSA public key");
        }

        return new MLDSAJWSVerifier(publicKey);
    }
